...
CORE = 7
SOURCE = "//Smurov/XML_ru/6635/" #"C:\Diploma\Original_data\Ivan\\6629\\" #
DESTINATION = "./CSVdata/15-03-16-without-numbers-1/6635/"
...
def loadSentence(element, sentence_id):
	...
	if sentence_text != '!Action Pact!': 	
		...
		for child in children:
			...
			loaded_child = loadChild(child[-2][-1], -1, sentence_id)
...
def loadChild(element, parent_id, sentence_id):
	...
	for morpho_feature in morpho_features:
		if morpho_feature.text != None:
			core.features[morpho_feature.tag] = morpho_feature.text.split(' | ')
	...
	if children != None:
		...
		for child in children:
			loaded_child = loadChild(child[-2][-1], core.id, sentence_id)
			if loaded_child != None:
				cores.extend(loaded_child)
			else:
				return None
